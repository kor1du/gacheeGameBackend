...
public class MemberService {
    ...
    private KakaoOAuthDto.UserInfo getKakaoUserInfo(String accessToken) {
        ...
        JsonNode nickname = kakaoAccount.get("profile").get("nickname");
        JsonNode email = kakaoAccount.get("email");
        JsonNode gender = kakaoAccount.get("gender");
        JsonNode profileImageUrl = kakaoAccount.get("profile").get("profile_image_url");

        return KakaoOAuthDto.UserInfo
                .builder()
                .userId(Long.parseLong(kakaoUserInfoJson.get("id").toPrettyString()))
                .nickname(nickname.textValue())
                .email(email == null ? "" : email.textValue())
                .gender(gender == null ? "" : gender.textValue())
                .profileImage(profileImageUrl == null ? "" : profileImageUrl.textValue())
                .build();
    }
